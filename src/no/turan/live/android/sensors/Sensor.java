...
public abstract class Sensor implements ISensor {
	...
	protected byte transmisstionType_ = 0;
	...
	public void connectionStateChanged(WFSensorConnectionStatus status) {
		if (sensor_ != null) {
			Log.d(TAG, "Sensor.connectionStateChanged - " + sensor_.getSensorType() 
					+ " - " + sensor_.getDeviceNumber() + " - " + status.name());
		} else {
		...
		if (sensor_ != null && sensor_.isValid()) {
			...
			case WF_SENSOR_CONNECTION_STATUS_CONNECTED:
				if (sensor_.isConnected()) {
					Log.d(TAG, "Sensor.connectionStateChanged - connected - " 
							+ sensor_.getSensorType() + " - " + sensor_.getDeviceNumber() 
							+ " - " + sensor_.getTransmissionType());
					sensorId_ = (short) sensor_.getDeviceNumber();
					transmisstionType_ = (byte) sensor_.getTransmissionType();
				}
			...
			Log.d(TAG, "Sensor.connectionStateChanged - " + sensor_.getSensorType() + " - " 
					+ sensor_.getDeviceNumber() + " - invalid");
	...
	public void connectSensor() {
		...
		if (hardwareConnector_ != null) {
			if (sensor_ != null) {
				Log.d(TAG, "Sensor.connectSensor - " + sensorType_ + " - " 
						+ sensor_.getDeviceNumber() + " - " + sensor_.getConnectionStatus());
				switch (sensor_.getConnectionStatus())
				{
					case WF_SENSOR_CONNECTION_STATUS_CONNECTED:
					case WF_SENSOR_CONNECTION_STATUS_CONNECTING:
			...
			if (sensor_ == null) {
				...
				deviceParam.transmissionType = transmisstionType_;
	...
	protected void deadSample() {
		...
		if (++deadSamples_ > DEAD_SAMPLE_THRESHOLD) {
			Log.d(TAG, "Sensor.deadSample threshold exceeded: " + sensor_.getSensorType() 
					+ " - " + sensor_.getDeviceNumber());
			disconnectSensor();
			connectSensor();
			reconnectable_ = true;
		}
